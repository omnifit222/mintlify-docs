---
title: "Integrate  OmnifitAI with Google Sheets"
description: "This page contains the stepwise tutorial for the integration of Google Sheets with  OmnifitAI"
---

### **Why Use  OmnifitAI with Google Sheets?**
...
The following conditions must be fulfilled to use Google Sheet Integration with  OmnifitAI:

1. OmnifitAI Account: The user must have an active  OmnifitAI Account, and must have an active bot which they wish to connect.
...
1. Login to your  OmnifitAI account. Now, select and click the chatbot you want to integrate with Google Sheets. Click on the “Third-party Apps Integration”.
...
1. Now Click On The **‘Connect’** Button As Shown
2. Click on the "**Connect**" button to continue to the integration.
3. Click on the "**Sign-in with Google**" to start the connection process.
...
2. Now, click allow for  OmnifitAI to access your Google account.
3. After you click allow, the connection between  OmnifitAI and Google Sheets is established, and you will be redirected back to  OmnifitAI onto the following page.

### **Step 3 of 4: Set up Mapping Parameters**

1. Now, you need to map **parameters (fields) of Google Sheets** to the user response of the  OmnifitAI question. In other words, you need to map the questions or custom attributes that will be saved to the Google Sheet. The fields that need to be entered are:
...
<Warning>
  Make sure you have a empty sheet made in your google sheet account, so it can be connect to  OmnifitAI.
</Warning>

You can use the **\+Add Parameters** button to add more fields.

For example, you select a question: Please provide your name?. Then, you map it with the parameter -Name. As a result, this mapping will enable integration to send the user's response from  OmnifitAI to the Google Sheet’s \*\*Name \*\*field in the leads. Similarly, in order for integration to work properly, you will require to map all other questions/ components of your Bot to Google Sheets fields.
...
Once you have established a connection between OmnifitAI and **Google Sheets**, and the mapping is complete, you are ready to go for your first test lead. You can test your integration setup by creating a dummy lead on the bot. This lead should appear in your  OmnifitAI dashboard as well as in the Google Sheets you have integrated.
...
<AccordionGroup>
  ...
  <Accordion title="How can I test whether my integration is working?">
    After setup, run a **dummy test lead** through the chatbot. If configured properly, the data should appear in your **Google Sheet** and also reflect in your OmnifitAI dashboard.
  </Accordion>
